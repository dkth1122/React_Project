...
export async function POST(req) {
  try {
    ...
    const searchResult = await new Promise((resolve, reject) => {
      db.query('SELECT * FROM likepost WHERE user_id = ? AND posts_no = ?', [userId, post], (err, result) => {
        if (err) {
    ...
    if (searchResult.length > 0) {
      ...
      const deleteResult = await new Promise((resolve, reject) => {
        db.query('DELETE FROM likepost WHERE user_id = ? AND posts_no = ?', [userId, post], (err, result) => {
          if (err) {
      ...
      const insertResult = await new Promise((resolve, reject) => {
        db.query('INSERT INTO likepost (user_id, posts_no) VALUES (?, ?)', [userId, post], (err, result) => {
          if (err) {
...
export async function GET(req) { 
  ...
  let sqlQuery = 'SELECT * FROM posts p LEFT JOIN likepost lk ON p.posts_no = lk.posts_no LEFT JOIN user u ON p.user_id = u.user_id LEFT JOIN profile f ON u.user_id = f.user_id WHERE lk.user_id = ?'
