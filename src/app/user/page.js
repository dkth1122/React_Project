...
export default function User(){
  ...
  useEffect(() => {
    fetch(`/api/follow?param1=${param1}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('서버 응답이 실패했습니다.');
        }
  ...
  useEffect(() => {
    fetch(`/api/following?param1=${param1}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('서버 응답이 실패했습니다.');
        }
  ...
  useEffect(() => {
    fetch(`/api/bookmark?param1=${param1}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('서버 응답이 실패했습니다.');
        }
  ...
  return (
    <div className='userAll'>
      <Container>
        <ProfileHeader>
            ...
            <Avatar src={posts.length > 0 ? posts[0].profile_img_path : '/expImg.png'}/>
    
          <UserInfo>
            <Username> 

              <span>{posts.length > 0 ? posts[0].user_nickname : ''}</span>

           </Username>
            <Bio>
              <span>{posts.length > 0 ? posts[0].bio : ''}</span>
           </Bio>
            <Stats>
              <StatItem>
                <StatLabel>팔로워</StatLabel>
                <StatValue>{follow.length > 0  ? follow[0].CNT : 0}</StatValue>
              </StatItem>
              <StatItem>
                <StatLabel>팔로잉</StatLabel>
                <StatValue>{following.length > 0  ? following[0].CNT : 0}</StatValue>
              </StatItem>
            </Stats>
    ...
    {activeTab === 'bookmark' && (
      <Posts>
        ...
        {activeTab === 'bookmark' && bookmark.length === 0 && (
          <div>북마크  한 피드가 없습니다.</div>
        )}
  </Container>
  <MenuBar />
</div>
